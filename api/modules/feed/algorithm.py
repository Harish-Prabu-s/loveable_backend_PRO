...
def _get_user_interest_profile(user):
    ...
    viewed_posts = PostView.objects.filter(user=user, created_at__gte=cutoff).values_list('post_id', flat=True)
    ...
    viewed_reels = ReelView.objects.filter(user=user, created_at__gte=cutoff).values_list('reel_id', flat=True)
